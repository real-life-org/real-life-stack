...
**`ProfilePanelContent`:** Geteilter Profil-Inhalt mit `mode: "edit" | "view"`. `edit` = eigenes Profil (Avatar-Upload, Name/Bio-Inputs, Positionsfeld über das `location`-Widget, Save); `view` = read-only Projektion (Avatar/Name/DID, Bio und Ort nur wenn vorhanden).

**Position im Editor:** Das Feld nutzt dasselbe `location`-Widget wie der Composer und schreibt `data.position` (GeoJSON Point) plus `data.locationName`. Es ist **opt-in** und trägt den Hinweis „Deine Position gilt in allen Spaces, in denen du Mitglied bist." ([04 §Profile](../04-items-relations-groups-spaces.md), Regel 4 — eine Position je Space gibt es nicht). Ein geleertes Feld ist eine Löschung, keine Auslassung: der Editor schickt beide Werte auch leer mit, und `updateMyProfile` verwaltet ein Feld genau dann, wenn der Aufrufer es nennt. Der Karten-Pick bleibt dem Composer vorbehalten — das Profil-Panel ist ein Modal mit Backdrop und müsste dafür beiseitetreten; die Adresssuche setzt die Position auch so. Wird ohne eigene Dialog-Hülle gerendert — die App Shell hängt ihn in ihre geteilte `AdaptivePanel`-Instanz.
