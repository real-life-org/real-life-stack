...
2. Ein Modul wird durch genau **einen** Registereintrag eingeführt. Das Toolkit **definiert** das Register (die Kern-Module, vollständig, samt Fläche); eine App **erweitert** es — mit eigenen Modulen oder, wenn sie will, mit einer ersetzten Fläche für ein Kern-Modul. **Definition vor Erweiterung**: Eine Erweiterung kann nur ergänzen, was schon eingeführt ist. Eine bereits vergebene `id` ist ein Konflikt und MUSS abgelehnt werden. Eine Erweiterung ergänzt einen vorhandenen Eintrag additiv; ein Feld, das die Definition bereits gesetzt hat, DARF sie nicht stillschweigend überschreiben — sie MUSS es ausdrücklich als Ersatz kennzeichnen, sonst ist es ein Konflikt, der die Zusammensetzung abbricht, mit Nennung des Feldes und beider Quellen. Es gibt kein Shadowing.

    *Bis zum 21.09.2026 hieß das hier „Schichten in der Reihenfolge Core → App". Das Wort versprach eine Architektur, die es nicht gibt: Es waren genau zwei Eingaben, und die zweite trug nur `view` nach — was mit dem Modul-Host entfällt. „Schicht" gehört Spec 00 (UI-Flächen, Hooks, DataInterface, Connector, Datenquelle); hier hat es nichts zu suchen. Der Mechanismus bleibt, sein Name nicht.*
