...
**Status: umgesetzt, rls#414 (B0, 21.09.2026).** `ModuleHost` in `packages/toolkit/src/components/host/module-host.tsx`; das Outlet rendert jede Fläche darin. Kalender und Karte laufen ohne eine Zeile in der App; die fünf übrigen Ansichten der Referenz-App lesen Items und Kontext vom Host und ziehen je in einem eigenen Schritt (B1–B5) ins Toolkit.
