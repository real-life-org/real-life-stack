...
sourceHash: ce8f4bb2f32965e4c81ad324ebf11e57a321575c86a48549960681d18c12547f
group: erweitern
sidebar:
  order: 11
sources:
...
`pnpm check:hooks` and `pnpm check:agents` check the same for the generated parts: an exported hook without a complete block, a story id that does not exist, a package without description, or a generated file that no longer matches the repository turns CI red. All of them run in `.github/workflows/tests.yml`; the overview is under [Work with agents → The checks](/en/handbuch/agenten/#the-checks).
