...
Eine Relation verbindet ein Item über ein Prädikat mit einem Ziel. Kommentare und Reaktionen sind eigene Items mit einer `commentOn`-Relation, nicht Arrays im Beitrag, die immer länger werden; Stellungnahmen im Resonanz-Modul sind signierte Relation-Records (Spec 08). Für jede Beziehungsart gibt es den passenden Hook (`useComments`, `useReactions`, `useVotes`, `useRelationRecords`); einen allgemeinen Rückwärts-Lookup gibt es absichtlich nicht.
