...
1. Den neuen Tag in einer Vorschau prüfen, mit eigenem Compose-Projektnamen und eigenem Port, damit sie den laufenden Container nicht ersetzt: `RLS_PORT=8081 RLS_IMAGE_TAG=0.4 docker compose -p rls-vorschau -f docker-compose.preview.yml up -d`. Danach `docker compose -p rls-vorschau -f docker-compose.preview.yml down`.
