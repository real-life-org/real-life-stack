...
Im Repo hängen die Pakete als `workspace:*` am Quelltext. Außerhalb des Repos trägst du die veröffentlichten Versionen von npm ein (`@real-life-stack/toolkit`, `@real-life-stack/data-interface`, `@real-life-stack/mock-connector`); der Rahmen ist ab der nächsten Veröffentlichung nach 0.1.10 enthalten.
...
`dev:first-app` baut zuerst die drei Pakete, an denen das Beispiel hängt (im Repo liegen sie als Quelltext, das Beispiel nimmt ihre gebaute Form), und startet dann Vite. Außerhalb des Repos genügt `pnpm dev`: Von npm kommen die Pakete gebaut.
