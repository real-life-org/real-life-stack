...
`pnpm check:hooks` und `pnpm check:agents` prüfen dasselbe für die erzeugten Teile: Ein exportierter Hook ohne vollständigen Block, eine Story-Id, die es nicht gibt, ein Paket ohne Beschreibung, oder eine erzeugte Datei, die nicht mehr zum Repo passt, macht CI rot. Alle laufen in `.github/workflows/tests.yml`; die Übersicht steht unter [Mit Agenten arbeiten → Die Prüfungen](/handbuch/agenten/#die-prüfungen).
