...
- **Alle sieben Toolkit-Module laufen ohne eine Zeile in der App** (seit 21.09.2026, B0–B5): Feed, Kanban, Kalender, Karte, Resonanz, Liste, Graph in `packages/toolkit/src/modules/`. Die Referenz-App erweitert das Register nicht mehr; eine App darf eigene Module einführen oder eine Fläche ersetzen — ausdrücklich, mit `replaces: ["view"]` am Fragment. Muster für ein neues Modul: eine Datei in `modules/`, ein Registereintrag, eine Story mit `HostWorld`.
