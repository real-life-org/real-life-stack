 ...
 * weil Starlights Hero sein Bild nur so entgegennimmt (`hero.image.html`). Nur das Telefon, im Maßstab der
 * Handbuchspalte, ohne Umschalter; das Skript `story-frames` belebt sie.
 */
export function heroPreviewHtml(lang: string): string {
  ...
  return (
    `<figure class="story-example hero-story not-content" data-frame="phone" data-scale="handbook">` +
