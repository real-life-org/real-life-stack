...
- [x] **apps-share-the-shell**: Was jede App mit Router gleich tut, liegt im Unterpfad `/router` (`useWorkspaceRouting`, `notificationRoute`) und im Toolkit (`ActivityPanelController`); eine App ist eine Shell um Register, Host-Provider und Outlet. Die Netzwerk-App läuft seit rls#429 so — mit einem eigenen Modul (Marktplatz über Hinweis `resource`) und `replaces: ["options"]` für Karte und Kalender
